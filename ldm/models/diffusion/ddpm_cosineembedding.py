...
#from deepface import DeepFace ## -- SUd
...
class LatentDiffusion(DDPM):
    ...
    def instantiate_first_stage(self, config):
        ...
        #self.first_stage_model.train = disabled_train ## it was uncommented originally SUD
        for param in self.first_stage_model.parameters():
            param.requires_grad = True ## it was False originally 
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        logvar_t = self.logvar[t.cpu()].to(self.device) #! Changed
        ...
        model_output_flatten = torch.flatten(model_output,start_dim=2) #.transpose([0,2,1])
        ...
        loss_cosine = self.cosine_weight * torch.stack(cosineloss,axis=0)
        loss_cosine = sum(loss_cosine)/len(loss_cosine)
        
        loss_dict.update({f'{prefix}/loss_cosine': loss_cosine})
        loss += loss_cosine
